...
public class OpenBankApiClient {
	...
	public BankResponseToken requestToken(BankRequestToken bankRequestToken) {
		...
		//Http 요청하기 - post 방식으로
		return restTemplate.exchange("https://testapi.openbanking.or.kr/oauth/2.0/token", HttpMethod.POST, param,
			BankResponseToken.class).getBody();
	}
	...
	public BankAccountSearchResponseDto requestAccountList(AccountSearchRequestDto accountSearchRequestDto) {
		...
		return restTemplate.exchange(builder.toUriString(), HttpMethod.GET, openBankAcountSerchRequest,
			BankAccountSearchResponseDto.class).getBody();
	}
	...
	public BankBalanceResponseDto requestBalance(BankBalanceRequestDto bankBalanceRequestDto, String access_token) {
		...
		bankBalanceRequestDto.setBankTransIdAndTranssDtime(
			openBankUtil.getRandomNumber(bankBalanceRequestDto.getBank_tran_id()),
		...
		return restTemplate.exchange(builder.toUriString(), HttpMethod.GET, balance, BankBalanceResponseDto.class)
			.getBody();
	}
	...
	public AccountTransferResponseDto requestTransfer(String access_token,
		...
		ResponseEntity<AccountTransferRequestDto> param = new ResponseEntity<>(accountTransferRequestDto,
			setHeader(access_token), HttpStatus.OK);
